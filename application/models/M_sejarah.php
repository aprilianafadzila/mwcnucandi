...
class M_sejarah extends CI_Model{
	...
	function get_berita_slider(){
		$hsl=$this->db->query("SELECT tbl_sejarah.*,DATE_FORMAT(tulisan_tanggal,'%d/%m/%Y') AS tanggal FROM tbl_sejarah where tulisan_img_slider='1' ORDER BY tulisan_id DESC");
		return $hsl;
	}
	function get_berita_home(){
		$hsl=$this->db->query("SELECT tbl_sejarah.*,DATE_FORMAT(tulisan_tanggal,'%d/%m/%Y') AS tanggal FROM tbl_sejarah ORDER BY tulisan_id DESC limit 4");
		return $hsl;
	}

	function berita_perpage($offset,$limit){
		$hsl=$this->db->query("SELECT tbl_sejarah.*,DATE_FORMAT(tulisan_tanggal,'%d/%m/%Y') AS tanggal FROM tbl_sejarah ORDER BY tulisan_id DESC limit $offset,$limit");
		return $hsl;
	}
	...
	function cari_berita($keyword){
		$hsl=$this->db->query("SELECT tbl_sejarah.*,DATE_FORMAT(tulisan_tanggal,'%d/%m/%Y') AS tanggal FROM tbl_sejarah WHERE tulisan_judul LIKE '%$keyword%' LIMIT 5");
		return $hsl;
	}
