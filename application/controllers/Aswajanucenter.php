<?php
class Aswajanucenter extends CI_Controller{
	function __construct(){
		parent::__construct();
		$this->load->model('m_tulisan');
			$this->load->model('m_aswajanucenter');
	...
	function index(){
		$jum=$this->m_aswajanucenter->berita();
        ...
        $limit=5;
        $config['base_url'] = base_url() . 'aswajanucenter/index/';
            ...
            $x['page'] =$this->pagination->create_links();
						$x['data']=$this->m_aswajanucenter->berita_perpage($offset,$limit);
						$x['category']=$this->db->get('tbl_kategori');
						$x['profil']=$this->m_profiltk->get_all_tulisan();
						$x['contact']=$this->m_datayayasan->get_all_datatk();
						$x['populer']=$this->db->query("SELECT * FROM tbl_aswajanucenter ORDER BY tulisan_views DESC LIMIT 5");
						$x['title'] = 'mwcnucandi';
			      ...
			      $x['keywords'] = 'aswajanucenter, informasi seputar aswaja nu center, aswajanucenter mwcnucandi, Aswaja Nu Center Sidoarjo. artikel aswaja nu center, berita aswaja nu center';
						$this->load->view('depan/v_aswajanucenter',$x);
	}
	function detail($slugs){
		$slug=htmlspecialchars($slugs,ENT_QUOTES);
		$query = $this->db->get_where('tbl_aswajanucenter', array('tulisan_slug' => $slug));
		if($query->num_rows() > 0){
			$b=$query->row_array();
			$kode=$b['tulisan_id'];
			$this->db->query("UPDATE tbl_aswajanucenter SET tulisan_views=tulisan_views+1 WHERE tulisan_id='$kode'");
			$data=$this->m_aswajanucenter->get_berita_by_kode($kode);
			$row=$data->row_array();
			$x['id']=$row['tulisan_id'];
			...
			$x['show_komentar']=$this->m_aswajanucenter->show_komentar_by_tulisan_id($kode);
			$x['category']=$this->db->get('tbl_kategori');
			$x['populer']=$this->db->query("SELECT * FROM tbl_aswajanucenter ORDER BY tulisan_views DESC LIMIT 5");
			$this->load->view('depan/v_blog_detail',$x);
		}else{
			redirect('aswajanucenter');
		}
	}

	function kategori(){
		 $kategori=str_replace("-"," ",$this->uri->segment(3));
		 $query = $this->db->query("SELECT tbl_aswajanucenter.*,DATE_FORMAT(tulisan_tanggal,'%d/%m/%Y') AS tanggal FROM tbl_aswajanucenter WHERE tulisan_kategori_nama LIKE '%$kategori%' ORDER BY tulisan_views DESC LIMIT 5");
		 if($query->num_rows() > 0){
			 $x['data']=$query;
			 $x['category']=$this->db->get('tbl_kategori');
 			 $x['populer']=$this->db->query("SELECT * FROM tbl_aswajanucenter ORDER BY tulisan_views DESC LIMIT 5");
    ...
    function search(){
        ...
        $query=$this->m_aswajanucenter->cari_berita($keyword);
				if($query->num_rows() > 0){
					$x['contact']=$this->m_datayayasan->get_all_datatk();
