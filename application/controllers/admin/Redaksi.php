...
class Redaksi extends CI_Controller{
	...
	function get_edit(){
		...
		$x['kat']=$this->m_kategori->get_all_kategori();
		$this->load->view('admin/v_edit_tulisanredaksi',$x);
	}
	            ...
	            if(!empty($_FILES['filefoto']['name']))
	            {
	                if ($this->upload->do_upload('filefoto'))
	                {
	                        ...
	                        $judul=strip_tags($this->input->post('xjudul'));
													...
													$user_id=$p['pengguna_id'];
													$user_nama=$p['pengguna_nama'];
													$this->m_redaksi->update_tulisan($tulisan_id,$judul,$isi,$kategori_id,$kategori_nama,$imgslider,$user_id,$user_nama,$gambar,$slug);
	                    ...
	                    redirect('admin/redaksi');
									...
									$user_id=$p['pengguna_id'];
									$user_nama=$p['pengguna_nama'];
									$this->m_redaksi->update_tulisan_tanpa_img($tulisan_id,$judul,$isi,$kategori_id,$kategori_nama,$imgslider,$user_id,$user_nama,$slug);
									echo $this->session->set_flashdata('msg','info');
									redirect('admin/redaksi');
	            }
	...
	function hapus_tulisan(){
		...
		unlink($path);
		$this->m_redaksi->hapus_tulisan($kode);
