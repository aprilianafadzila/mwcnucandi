...
class Sejarah extends CI_Controller{
	...
	function index(){
        ...
        $config['base_url'] = base_url() . 'sejarah/index/';
            ...
            $x['page'] =$this->pagination->create_links();
						$x['data']=$this->m_sejarah->berita_perpage($offset,$limit);
		        ...
		        $x['keywords'] = 'sejarah mwcnucandi, sejarah Sidoarjo, sejarah nu di candi, artikel tentang sejarah, berita tentang sejarah, sejarah candi';
						$this->load->view('depan/v_sejarah',$x);
	}
	function detail($slugs){
		$slug=htmlspecialchars($slugs,ENT_QUOTES);
		$query = $this->db->get_where('tbl_sejarah', array('tulisan_slug' => $slug));
		if($query->num_rows() > 0){
			$b=$query->row_array();
			$kode=$b['tulisan_id'];
			...
			$x['show_komentar']=$this->m_sejarah->show_komentar_by_tulisan_id($kode);
			$x['category']=$this->db->get('tbl_kategori');
			$x['populer']=$this->db->query("SELECT * FROM tbl_sejarah ORDER BY tulisan_views DESC LIMIT 5");
	...
	function kategori(){
		 ...
		 $query = $this->db->query("SELECT tbl_sejarah.*,DATE_FORMAT(tulisan_tanggal,'%d/%m/%Y') AS tanggal FROM tbl_sejarah WHERE tulisan_kategori_nama LIKE '%$kategori%' ORDER BY tulisan_views DESC LIMIT 5");
		 if($query->num_rows() > 0){
			 $x['data']=$query;
			 $x['category']=$this->db->get('tbl_kategori');
 			 $x['populer']=$this->db->query("SELECT * FROM tbl_sejarah ORDER BY tulisan_views DESC LIMIT 5");
    ...
    function search(){
        ...
        $query=$this->m_tulisan->cari_berita($keyword);
				if($query->num_rows() > 0){
					$x['contact']=$this->m_datayayasan->get_all_datatk();
					$x['all_sejarah']=$this->m_sejarah->get_all_sejarah();
					$x['data']=$query;
					$x['category']=$this->db->get('tbl_kategori');
  				$x['populer']=$this->db->query("SELECT * FROM tbl_sejarah ORDER BY tulisan_views DESC LIMIT 5");
          $this->load->view('depan/v_sejarah',$x);
