...
class Lkknu extends CI_Controller{
	...
	function index(){
        ...
        $config['base_url'] = base_url() . 'lkknu/index/';
            ...
            $x['page'] =$this->pagination->create_links();
						$x['data']=$this->m_lkknu->berita_perpage($offset,$limit);
			      ...
			      $x['keywords'] = 'lkknu, informasi lkknu candi, berita lkknu candi, artikel lkknu candi, berita terbaru lkknu candi, lkknu mwcnucandi Sidoarjo';
						$this->load->view('depan/v_lkknu',$x);
	}
	function detail($slugs){
		$slug=htmlspecialchars($slugs,ENT_QUOTES);
		$query = $this->db->get_where('tbl_lkknu', array('tulisan_slug' => $slug));
		if($query->num_rows() > 0){
			$b=$query->row_array();
			$kode=$b['tulisan_id'];
			$this->db->query("UPDATE tbl_lkknu SET tulisan_views=tulisan_views+1 WHERE tulisan_id='$kode'");
			$data=$this->m_lkknu->get_berita_by_kode($kode);
			$row=$data->row_array();
			$x['id']=$row['tulisan_id'];
			...
			$x['show_komentar']=$this->m_lkknu->show_komentar_by_tulisan_id($kode);
			$x['category']=$this->db->get('tbl_kategori');
			$x['populer']=$this->db->query("SELECT * FROM tbl_lkknu ORDER BY tulisan_views DESC LIMIT 5");
	...
	function kategori(){
		 ...
		 $query = $this->db->query("SELECT tbl_lkknu.*,DATE_FORMAT(tulisan_tanggal,'%d/%m/%Y') AS tanggal FROM tbl_lkknu WHERE tulisan_kategori_nama LIKE '%$kategori%' ORDER BY tulisan_views DESC LIMIT 5");
		 if($query->num_rows() > 0){
			 $x['data']=$query;
			 $x['category']=$this->db->get('tbl_kategori');
 			 $x['populer']=$this->db->query("SELECT * FROM tbl_lkknu ORDER BY tulisan_views DESC LIMIT 5");
    ...
    function search(){
        ...
        $query=$this->m_lkknu->cari_berita($keyword);
				if($query->num_rows() > 0){
					$x['contact']=$this->m_datayayasan->get_all_datatk();
					$x['all_galeri']=$this->m_galeri->get_all_galeri();
					$x['data']=$query;
