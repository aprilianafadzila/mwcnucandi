...
class Ansor extends CI_Controller{
	...
	function index(){
        ...
        $limit=5;
        $config['base_url'] = base_url() . 'ansor/index/';
            ...
            $x['page'] =$this->pagination->create_links();
						...
						$x['data']=$this->m_ansor->berita_perpage($offset,$limit);
						...
						$x['populer']=$this->db->query("SELECT * FROM tbl_ansor ORDER BY tulisan_views DESC LIMIT 5");
						$x['title'] = 'mwcnucandi';
			      ...
			      $x['keywords'] = 'amsor candi, gp ansor candi, berita ansor candi, berita terbaru ansor candi, artikel ansor candi, tulisan ansor candi, acara ansor candi, informasi  ansor candi, ansor mwcnucandi Sidoarjo';
						$this->load->view('depan/v_ansor',$x);
	}
	function detail($slugs){
		$slug=htmlspecialchars($slugs,ENT_QUOTES);
		$query = $this->db->get_where('tbl_ansor', array('tulisan_slug' => $slug));
		if($query->num_rows() > 0){
			$b=$query->row_array();
			$kode=$b['tulisan_id'];
			$this->db->query("UPDATE tbl_ansor SET tulisan_views=tulisan_views+1 WHERE tulisan_id='$kode'");
			$data=$this->m_ansor->get_berita_by_kode($kode);
			$row=$data->row_array();
			$x['id']=$row['tulisan_id'];
			...
			$x['show_komentar']=$this->m_ansor->show_komentar_by_tulisan_id($kode);
			$x['category']=$this->db->get('tbl_kategori');
			$x['populer']=$this->db->query("SELECT * FROM tbl_ansor ORDER BY tulisan_views DESC LIMIT 5");
	...
	function kategori(){
		 ...
		 $query = $this->db->query("SELECT tbl_ansor.*,DATE_FORMAT(tulisan_tanggal,'%d/%m/%Y') AS tanggal FROM tbl_ansor WHERE tulisan_kategori_nama LIKE '%$kategori%' ORDER BY tulisan_views DESC LIMIT 5");
		 if($query->num_rows() > 0){
			 $x['data']=$query;
			 $x['category']=$this->db->get('tbl_kategori');
 			 $x['populer']=$this->db->query("SELECT * FROM tbl_ansor ORDER BY tulisan_views DESC LIMIT 5");
    ...
    function search(){
        ...
        $query=$this->m_ansor->cari_berita($keyword);
				if($query->num_rows() > 0){
					$x['contact']=$this->m_datayayasan->get_all_datatk();
					$x['all_galeri']=$this->m_galeri->get_all_galeri();
					$x['data']=$query;
