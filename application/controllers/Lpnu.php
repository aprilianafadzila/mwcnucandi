<?php
class Lpnu extends CI_Controller{
	function __construct(){
		parent::__construct();
		$this->load->model('m_lpnu');
	...
	function index(){
        ...
        $config['base_url'] = base_url() . 'blog/index/';
            ...
            $x['page'] =$this->pagination->create_links();
						$x['data']=$this->m_lpnu->berita_perpage($offset,$limit);
						...
						$x['populer']=$this->db->query("SELECT * FROM tbl_lpnu ORDER BY tulisan_views DESC LIMIT 5");
						$x['title'] = 'mwcnucandi';
			      ...
			      $x['keywords'] = 'lpnu, informasi lpnu candi, berita lpnu candi, artikel lpnu candi, berita terbaru lpnu candi, lpnu mwcnucandi Sidoarjo';
						$this->load->view('depan/v_blog',$x);
	}
	function detail($slugs){
		$slug=htmlspecialchars($slugs,ENT_QUOTES);
		$query = $this->db->get_where('tbl_lpnu', array('tulisan_slug' => $slug));
		if($query->num_rows() > 0){
			$b=$query->row_array();
			$kode=$b['tulisan_id'];
			$this->db->query("UPDATE tbl_lpnu SET tulisan_views=tulisan_views+1 WHERE tulisan_id='$kode'");
			$data=$this->m_lpnu->get_berita_by_kode($kode);
			$row=$data->row_array();
			$x['id']=$row['tulisan_id'];
			...
			$x['show_komentar']=$this->m_lpnu->show_komentar_by_tulisan_id($kode);
			$x['category']=$this->db->get('tbl_kategori');
			$x['populer']=$this->db->query("SELECT * FROM tbl_lpnu ORDER BY tulisan_views DESC LIMIT 5");
	...
	function kategori(){
		 ...
		 $query = $this->db->query("SELECT tbl_lpnu.*,DATE_FORMAT(tulisan_tanggal,'%d/%m/%Y') AS tanggal FROM tbl_lpnu WHERE tulisan_kategori_nama LIKE '%$kategori%' ORDER BY tulisan_views DESC LIMIT 5");
		 if($query->num_rows() > 0){
			 $x['data']=$query;
			 $x['category']=$this->db->get('tbl_kategori');
 			 $x['populer']=$this->db->query("SELECT * FROM tbl_lpnu ORDER BY tulisan_views DESC LIMIT 5");
    ...
    function search(){
        ...
        $query=$this->m_lpnu->cari_berita($keyword);
				if($query->num_rows() > 0){
					$x['contact']=$this->m_datayayasan->get_all_datatk();
					$x['all_galeri']=$this->m_galeri->get_all_galeri();
					$x['data']=$query;
