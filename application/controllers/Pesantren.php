...
class Pesantren extends CI_Controller{
	...
	function index(){
        ...
        $limit=5;
        $config['base_url'] = base_url() . 'pesantren/index/';
            ...
            $x['page'] =$this->pagination->create_links();
						$x['data']=$this->m_pesantren->berita_perpage($offset,$limit);
						$x['category']=$this->db->get('tbl_kategori');
						$x['profil']=$this->m_profiltk->get_all_tulisan();
						$x['contact']=$this->m_datayayasan->get_all_datatk();
						$x['populer']=$this->db->query("SELECT * FROM tbl_pesantren ORDER BY tulisan_views DESC LIMIT 5");
						$x['title'] = 'mwcnucandi';
		        ...
		        $x['keywords'] = 'pesantren, pesantren mwcnucandi, pesantren di Sidoarjo, berita tentang pesantren';
						$this->load->view('depan/v_pesantren',$x);
	}
	function detail($slugs){
		$slug=htmlspecialchars($slugs,ENT_QUOTES);
		$query = $this->db->get_where('tbl_pesantren', array('tulisan_slug' => $slug));
		if($query->num_rows() > 0){
			$b=$query->row_array();
			$kode=$b['tulisan_id'];
			$this->db->query("UPDATE tbl_pesantren SET tulisan_views=tulisan_views+1 WHERE tulisan_id='$kode'");
			$data=$this->m_pesantren->get_berita_by_kode($kode);
			$row=$data->row_array();
			$x['id']=$row['tulisan_id'];
			...
			$x['show_komentar']=$this->m_pesantren->show_komentar_by_tulisan_id($kode);
			$x['category']=$this->db->get('tbl_kategori');
			$x['populer']=$this->db->query("SELECT * FROM tbl_pesantren ORDER BY tulisan_views DESC LIMIT 5");
	...
	function kategori(){
		 ...
		 $query = $this->db->query("SELECT tbl_pesantren.*,DATE_FORMAT(tulisan_tanggal,'%d/%m/%Y') AS tanggal FROM tbl_pesantren WHERE tulisan_kategori_nama LIKE '%$kategori%' ORDER BY tulisan_views DESC LIMIT 5");
		 if($query->num_rows() > 0){
			 $x['data']=$query;
			 $x['category']=$this->db->get('tbl_kategori');
 			 $x['populer']=$this->db->query("SELECT * FROM tbl_pesantren ORDER BY tulisan_views DESC LIMIT 5");
    ...
    function search(){
        ...
        $query=$this->m_pesantren->cari_berita($keyword);
				if($query->num_rows() > 0){
					$x['contact']=$this->m_datayayasan->get_all_datatk();
					$x['all_galeri']=$this->m_pesantren->get_all_pesantren();
					$x['data']=$query;
					$x['category']=$this->db->get('tbl_kategori');
  				$x['populer']=$this->db->query("SELECT * FROM tbl_pesantren ORDER BY tulisan_views DESC LIMIT 5");
          $this->load->view('depan/v_pesantren',$x);
