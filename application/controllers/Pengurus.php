...
class Pengurus extends CI_Controller{
	...
	function index(){
        ...
        $limit=5;
        $config['base_url'] = base_url() . 'pengurus/index/';
            ...
            $x['page'] =$this->pagination->create_links();
						$x['data']=$this->m_pengurus->berita_perpage($offset,$limit);
	...
	function detail($slugs){
		...
		$query = $this->db->get_where('tbl_pengurus', array('tulisan_slug' => $slug));
		if($query->num_rows() > 0){
			$b=$query->row_array();
			$kode=$b['tulisan_id'];
			$this->db->query("UPDATE tbl_pengurus SET tulisan_views=tulisan_views+1 WHERE tulisan_id='$kode'");
			$data=$this->m_pengurus->get_berita_by_kode($kode);
			$row=$data->row_array();
			$x['id']=$row['tulisan_id'];
			...
			$x['show_komentar']=$this->m_pengurus->show_komentar_by_tulisan_id($kode);
			$x['category']=$this->db->get('tbl_kategori');
			$x['populer']=$this->db->query("SELECT * FROM tbl_pengurus ORDER BY tulisan_views DESC LIMIT 5");
	...
	function kategori(){
		 ...
		 $query = $this->db->query("SELECT tbl_pengurus.*,DATE_FORMAT(tulisan_tanggal,'%d/%m/%Y') AS tanggal FROM tbl_pengurus WHERE tulisan_kategori_nama LIKE '%$kategori%' ORDER BY tulisan_views DESC LIMIT 5");
		 if($query->num_rows() > 0){
			 $x['data']=$query;
			 $x['category']=$this->db->get('tbl_kategori');
 			 $x['populer']=$this->db->query("SELECT * FROM tbl_pengurus ORDER BY tulisan_views DESC LIMIT 5");
    ...
    function search(){
        ...
        $query=$this->m_tulisan->cari_berita($keyword);
				if($query->num_rows() > 0){
					$x['contact']=$this->m_datayayasan->get_all_datatk();
					$x['all_pengurus']=$this->m_pengurus->get_all_pengurus();
					$x['data']=$query;
					$x['category']=$this->db->get('tbl_kategori');
  				$x['populer']=$this->db->query("SELECT * FROM tbl_pengurus ORDER BY tulisan_views DESC LIMIT 5");
          $this->load->view('depan/v_pengurus',$x);
