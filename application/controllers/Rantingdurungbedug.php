<?php
class Rantingdurungbedug extends CI_Controller{
	function __construct(){
		parent::__construct();
		$this->load->model('m_tulisan');
			$this->load->model('m_galeri');
			$this->load->model('m_profiltk');
			$this->load->model('m_rantingdurungbedug');
	...
	function index(){
		$jum=$this->m_rantingdurungbedug->berita();
        ...
        $limit=5;
        $config['base_url'] = base_url() . 'rantingdurungbedug/index/';
            ...
            $x['page'] =$this->pagination->create_links();
						$x['data']=$this->m_rantingdurungbedug->berita_perpage($offset,$limit);
	...
	function detail($slugs){
		...
		$query = $this->db->get_where('tbl_rantingdurungbedug', array('tulisan_slug' => $slug));
		if($query->num_rows() > 0){
			$b=$query->row_array();
			$kode=$b['tulisan_id'];
			$this->db->query("UPDATE tbl_rantingdurungbedug SET tulisan_views=tulisan_views+1 WHERE tulisan_id='$kode'");
			$data=$this->m_rantingdurungbedug->get_berita_by_kode($kode);
			$row=$data->row_array();
			$x['id']=$row['tulisan_id'];
			$x['title']=$row['tulisan_judul'];
			$x['image']=$row['tulisan_gambar'];
			$x['rantingdurungbedug'] =$row['tulisan_isi'];
			$x['tanggal']=$row['tanggal'];
			$x['author']=$row['tulisan_author'];
			$x['kategori']=$row['tulisan_kategori_nama'];
			$x['slug']=$row['tulisan_slug'];
			$x['show_komentar']=$this->m_rantingdurungbedug->show_komentar_by_tulisan_id($kode);
			$x['category']=$this->db->get('tbl_kategori');
			$x['populer']=$this->db->query("SELECT * FROM tbl_rantingdurungbedug ORDER BY tulisan_views DESC LIMIT 5");
	...
	function kategori(){
		 ...
		 $query = $this->db->query("SELECT tbl_rantingdurungbedug.*,DATE_FORMAT(tulisan_tanggal,'%d/%m/%Y') AS tanggal FROM tbl_rantingdurungbedug WHERE tulisan_kategori_nama LIKE '%$kategori%' ORDER BY tulisan_views DESC LIMIT 5");
		 if($query->num_rows() > 0){
			 $x['data']=$query;
			 $x['category']=$this->db->get('tbl_kategori');
 			 $x['populer']=$this->db->query("SELECT * FROM tbl_rantingdurungbedug ORDER BY tulisan_views DESC LIMIT 5");
    ...
    function search(){
        ...
        $query=$this->m_rantingdurungbedug->cari_berita($keyword);
				if($query->num_rows() > 0){
					$x['contact']=$this->m_datayayasan->get_all_datatk();
					$x['all_galeri']=$this->m_galeri->get_all_galeri();
					$x['data']=$query;
