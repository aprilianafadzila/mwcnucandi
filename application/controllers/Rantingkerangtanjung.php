<?php
class Rantingkerangtanjung extends CI_Controller{
	function __construct(){
		parent::__construct();
		$this->load->model('m_rantingkerangtanjung');
	...
	function index(){
		$jum=$this->m_rantingkerangtanjung->berita();
        ...
        $limit=5;
        $config['base_url'] = base_url() . 'rantingkerangtanjung/index/';
            ...
            $x['page'] =$this->pagination->create_links();
						$x['data']=$this->m_rantingkerangtanjung->berita_perpage($offset,$limit);
						...
						$x['populer']=$this->db->query("SELECT * FROM tbl_rantingkerangtanjung ORDER BY tulisan_views DESC LIMIT 5");
						$x['title'] = 'mwcnucandi';
			      ...
			      $x['keywords'] = 'ranting kerangtanjung candi, acara ranting kerangtanjung candi, berita ranting kerangtanjung candi, berita terbaru ranting kerangtanjung candi, artikel ranting kerangtanjung candi, acara ranting kerangtanjung candi, tulisan ranting kerangtanjung candi, informasi ranting kerangtanjung candi, ranting kerangtanjung mwcnucandi Sidoarjo';
						$this->load->view('depan/v_rantingkerangtanjung',$x);
	}
	function detail($slugs){
		$slug=htmlspecialchars($slugs,ENT_QUOTES);
		$query = $this->db->get_where('tbl_rantingkerangtanjung', array('tulisan_slug' => $slug));
		if($query->num_rows() > 0){
			$b=$query->row_array();
			$kode=$b['tulisan_id'];
			$this->db->query("UPDATE tbl_rantingkerangtanjung SET tulisan_views=tulisan_views+1 WHERE tulisan_id='$kode'");
			$data=$this->m_rantingkerangtanjung->get_berita_by_kode($kode);
			$row=$data->row_array();
			$x['id']=$row['tulisan_id'];
			$x['title']=$row['tulisan_judul'];
			$x['image']=$row['tulisan_gambar'];
			$x['rantingkerangtanjung'] =$row['tulisan_isi'];
			$x['tanggal']=$row['tanggal'];
			$x['author']=$row['tulisan_author'];
			$x['kategori']=$row['tulisan_kategori_nama'];
			$x['slug']=$row['tulisan_slug'];
			$x['show_komentar']=$this->m_rantingkerangtanjung->show_komentar_by_tulisan_id($kode);
			$x['category']=$this->db->get('tbl_kategori');
			$x['populer']=$this->db->query("SELECT * FROM tbl_rantingkerangtanjung ORDER BY tulisan_views DESC LIMIT 5");
	...
	function kategori(){
		 ...
		 $query = $this->db->query("SELECT tbl_rantingkerangtanjung.*,DATE_FORMAT(tulisan_tanggal,'%d/%m/%Y') AS tanggal FROM tbl_rantingkerangtanjung WHERE tulisan_kategori_nama LIKE '%$kategori%' ORDER BY tulisan_views DESC LIMIT 5");
		 if($query->num_rows() > 0){
			 $x['data']=$query;
			 $x['category']=$this->db->get('tbl_kategori');
 			 $x['populer']=$this->db->query("SELECT * FROM tbl_rantingkerangtanjung ORDER BY tulisan_views DESC LIMIT 5");
    ...
    function search(){
        ...
        $query=$this->m_rantingkerangtanjung->cari_berita($keyword);
				if($query->num_rows() > 0){
					$x['contact']=$this->m_datayayasan->get_all_datatk();
					$x['all_galeri']=$this->m_galeri->get_all_galeri();
					$x['data']=$query;
