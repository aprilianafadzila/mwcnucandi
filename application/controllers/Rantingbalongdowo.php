...
class Rantingbalongdowo extends CI_Controller{
	...
	function index(){
        ...
        $limit=5;
        $config['base_url'] = base_url() . 'rantingbalongdowo/index/';
            ...
            $x['page'] =$this->pagination->create_links();
						...
						$x['keywords'] = 'ranting balongdowo candi, acara ranting balongdowo candi, berita ranting balongdowo candi, berita terbaru ranting balongdowo candi, artikel ranting balongdowo candi, acara ranting balongdowo candi, tulisan ranting balongdowo candi, informasi ranting balongdowo candi, ranting balongdowo mwcnucandi Sidoarjo';
						$this->load->view('depan/v_rantingbalongdowo',$x);
	}
	function detail($slugs){
		$slug=htmlspecialchars($slugs,ENT_QUOTES);
		$query = $this->db->get_where('tbl_rantingbalongdowo', array('tulisan_slug' => $slug));
		if($query->num_rows() > 0){
			$b=$query->row_array();
			$kode=$b['tulisan_id'];
			...
			$x['rantingbalongdowo'] =$row['tulisan_isi'];
	...
	function kategori(){
		 ...
		 $query = $this->db->query("SELECT tbl_rantingbalongdowo.*,DATE_FORMAT(tulisan_tanggal,'%d/%m/%Y') AS tanggal FROM tbl_rantingbalongdowo WHERE tulisan_kategori_nama LIKE '%$kategori%' ORDER BY tulisan_views DESC LIMIT 5");
		 if($query->num_rows() > 0){
			 $x['data']=$query;
			 $x['category']=$this->db->get('tbl_kategori');
 			 $x['populer']=$this->db->query("SELECT * FROM tbl_rantingbalongdowo ORDER BY tulisan_views DESC LIMIT 5");
    ...
    function search(){
        ...
        $query=$this->n_rantingbalongdowo->cari_berita($keyword);
				if($query->num_rows() > 0){
					$x['contact']=$this->m_datayayasan->get_all_datatk();
					$x['all_galeri']=$this->m_galeri->get_all_galeri();
					$x['data']=$query;
