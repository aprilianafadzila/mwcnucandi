<?php
class Lakpesdam extends CI_Controller{
	function __construct(){
		parent::__construct();
		$this->load->model('m_lakpesdam');
	...
	function index(){
        ...
        $limit=5;
        $config['base_url'] = base_url() . 'lakpesdam/index/';
            ...
            $x['page'] =$this->pagination->create_links();
						$x['data']=$this->m_lakpesdam->berita_perpage($offset,$limit);
						...
						$x['populer']=$this->db->query("SELECT * FROM tbl_lakpesdam ORDER BY tulisan_views DESC LIMIT 5");
						$x['title'] = 'mwcnucandi';
			      ...
			      $x['keywords'] = 'lakpesdam, lakpesdam candi, informasi lakpesdam, berita terbaru lakpesdam, artikel terbaru lakpesdam, Lakpesdam Sidoarjo';
						$this->load->view('depan/v_blog',$x);
	}
	function detail($slugs){
		$slug=htmlspecialchars($slugs,ENT_QUOTES);
		$query = $this->db->get_where('tbl_lakpesdam', array('tulisan_slug' => $slug));
		if($query->num_rows() > 0){
			$b=$query->row_array();
			$kode=$b['tulisan_id'];
			$this->db->query("UPDATE tbl_lakpesdam SET tulisan_views=tulisan_views+1 WHERE tulisan_id='$kode'");
			$data=$this->m_lakpesdam->get_berita_by_kode($kode);
			$row=$data->row_array();
			$x['id']=$row['tulisan_id'];
			...
			$x['show_komentar']=$this->m_lakpesdam->show_komentar_by_tulisan_id($kode);
			$x['category']=$this->db->get('tbl_kategori');
			$x['populer']=$this->db->query("SELECT * FROM tbl_lakpesdam ORDER BY tulisan_views DESC LIMIT 5");
	...
	function kategori(){
		 ...
		 $query = $this->db->query("SELECT tbl_lakpesdam.*,DATE_FORMAT(tulisan_tanggal,'%d/%m/%Y') AS tanggal FROM tbl_lakpesdam WHERE tulisan_kategori_nama LIKE '%$kategori%' ORDER BY tulisan_views DESC LIMIT 5");
		 if($query->num_rows() > 0){
			 $x['data']=$query;
			 $x['category']=$this->db->get('tbl_kategori');
 			 $x['populer']=$this->db->query("SELECT * FROM tbl_lakpesdam ORDER BY tulisan_views DESC LIMIT 5");
    ...
    function search(){
        ...
        $query=$this->m_lakpesdam->cari_berita($keyword);
				if($query->num_rows() > 0){
					$x['contact']=$this->m_datayayasan->get_all_datatk();
					$x['all_galeri']=$this->m_galeri->get_all_galeri();
					$x['data']=$query;
